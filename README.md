...
5. **AgentOS UI.** Chat with agents, inspect sessions, traces, memory, and evals from the control plane at [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-railway&utm_content=agentos-railway&utm_term=railway).
...
6. Ask me which frontends I want connected, then set up the ones I pick:
   ...
   - Claude and ChatGPT apps (web or desktop): their sessions run in the cloud and can't reach localhost, so work with me on deploying to production first using ./scripts/railway/up.sh (needs the Railway CLI, logged in; the script pauses while I mint a JWT key at os.agno.com, and saves the MCP_CONNECT_SECRET used in the OAuth consent secret in .env.production). Then I add https://<railway-domain>/mcp as a custom connector in the chat app's connector settings and approve the consent page with that secret.
...
For **claude.ai and ChatGPT (web)**: add `https://<railway-domain>/mcp` as a custom connector in the chat app's connector settings. `up.sh` would have generated a login secret for you during deploy (`MCP_CONNECT_SECRET`, also saved in `.env.production`).

Add `https://<railway-domain>/mcp` as a custom connector in the chat app's connector settings. **Leave the form's optional OAuth fields (client ID / client secret) empty**. Click **Connect** and on the consent page, enter the `MCP_CONNECT_SECRET` generated by `up.sh` (saved in `.env.production`)
